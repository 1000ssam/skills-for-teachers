...
> 📦 **이 리포는 공유 빌드(`ppt-grid-deck`)다** — 완성본 32 룩 + 빌드 엔진만 담은 소비용. 룩 카탈로그를 키우는 흡수 R&D(extract 도구·흡수 루틴 등)는 별도 연구 하네스 `ppt-lab-rebuild`에 있다. 아래 방법론·출처 크레딧은 두 빌드에 공통.
...
> ⚠️ **자작 범위 = 그리드 좌표계·도출 방법론·명명·씨앗 아키타입(24)에 한정.** 그 위 **레이아웃·스타일 카탈로그는 레퍼런스 흡수로 성장**한다(흡수된 레이아웃도 항상 이 자작 그리드 위에 앉는다). 흡수 자산의 출처·라이선스는 [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md). 즉 "클린룸"은 **그리드 좌표계 한정** 표현이지, "레퍼런스를 흡수하지 않는다"는 뜻이 아니다.
...
- design-pick 룩 — **MIT** (출처표기 후 사용). *이 공유본은 그중 완성본 32 룩을 번들*.
- velis 테마 — **CC0 1.0** (퍼블릭도메인).
- 레퍼런스 덱에서 흡수한 **스타일·룩**(design-diversity 팩 등)과 *진짜 새 구조*로 흡수한 **레이아웃**은 위 두 항목과 같이 출처·라이선스를 `THIRD-PARTY-NOTICES.md`에 표기한다. 흡수는 *스킨·기하 측정 + (1차)prompt.md 명세 번역*이며, 자작 그리드 좌표계 자체는 복제하지 않는다(= 그 한정 의미의 "클린룸").
