...
- **단일 모드에서 압축이나 보강이 발생할 때는 반드시 사용자 승인을 받은 뒤 최종 출력**한다 (Step 6). 자동 통과 금지.
...
3명 처리 후 각 학생의 결과 파일을 `output/세특_<이름>.txt`에 저장한다.
...
나머지 학생을 **10명 단위 chunk**로 나눈다. 각 chunk를 1개의 subagent에 할당. 총 `ceil((N-3)/10)`개 subagent 생성.
...
- 작업 지시:
  1. 학생별로 4단계 구조 초안 작성
  2. `byte_count.py`로 측정
  3. 한도 초과 시 **압축 1회만** 시도. 미달성이어도 그대로 진행
  4. 결과를 `output/세특_<이름>.txt`에 저장 (Write 도구)
  5. 처리 학생 메타(이름, 파일 경로, 바이트, 상태)를 반환
...
스크립트는 `output/세특_*.txt` 모두를 스캔하여 `manifest.csv`를 단일 writer로 작성한다. stdout은 통계 한 줄만 반환:
...
- **A 방침**: manifest 통계 요약 + 한도 초과·실패 학생 이름만 메인이 보고. 본문은 보고하지 않음.
  > "100명 처리 완료. 한도 초과 11명 (홍길동, 이영희, ...), 실패 2명 (박민수: 빈 입력, ...). 결과는 `output/manifest.csv`와 `output/세특_<이름>.txt`에서 확인하세요."
- **B 방침**: 위와 동일한 요약 + 사용자 검토 안내.
  > "처리 완료. 결과를 직접 확인 부탁드립니다. 수정이 필요한 학생을 알려주시면 개별 처리합니다."

### Batch Mode Invariants (반드시 준수)

1. **본문은 메인 컨텍스트로 흐르지 않는다**: 모든 학생 본문은 `output/세특_<이름>.txt`에만 존재. 메인은 메타데이터(이름·경로·바이트·상태)만 보유.
2. **한 파일 한 writer**: 학생별 .txt는 그 학생을 처리하는 단일 주체(메인 또는 특정 subagent)만 작성. 동시 쓰기 금지.
3. **manifest.csv는 메인이 1회만 작성**: 모든 subagent 종료 후 merge 스크립트로 생성.
4. **압축 1회 제한**: Batch Mode에서는 압축 반복 금지. 한도 미달성이어도 OVER 표기 후 진행.
5. **결과 파일을 Read하지 않는다**: 메인은 `cat`/`Read`로 본문을 다시 컨텍스트에 끌어오지 않음. 검증·조회는 `grep`, `awk`, `wc` 같은 부분 도구로만.
6. **Stdout 요약 원칙**: 모든 스크립트는 본문을 stdout으로 출력하지 않음. 통계·경로·카운트만.
7. **Subagent 자율성 제한**: subagent는 SKILL.md 규칙을 따르되 메인에게 질문할 수 없음. 의사결정 필요 시 OVER/ERROR로 표기 후 진행.
